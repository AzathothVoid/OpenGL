...
int main() {
	...
	orangeShader = glCreateShader(GL_FRAGMENT_SHADER);// creates a shader and stores unique ID in fragmentShader that defines it
	glShaderSource(orangeShader, 1, &fragmentShaderSource1, NULL);// attaches fragment shader code to fragmentShader variable
	glCompileShader(orangeShader); // compiles the fragment shader just created

	unsigned int yellowShader;
	yellowShader = glCreateShader(GL_FRAGMENT_SHADER);// creates a shader and stores unique ID in fragmentShader that defines it
	glShaderSource(yellowShader, 1, &fragmentShaderSource2, NULL);// attaches fragment shader code to fragmentShader variable
	glCompileShader(yellowShader); // compiles the fragment shader just created

	//checks if the framgnet shader encounters an error in compilation
	int success2;
	char infoLog2[512];
	glGetShaderiv(orangeShader, GL_COMPILE_STATUS, &success2);
	if (!success2) {
		glGetShaderInfoLog(orangeShader, 512, NULL, infoLog2);
	...
	glAttachShader(shaderProgram1, orangeShader);// attaches fragment shader to the shader program
	glLinkProgram(shaderProgram1); // links the attached shaders together

	unsigned int shaderProgram2;
	shaderProgram2 = glCreateProgram(); // creates a shader program and stores its unique ID in shaderProgram
	glAttachShader(shaderProgram2, vertexShader); // attaches vertex shader to the shader program
	glAttachShader(shaderProgram2, yellowShader);// attaches fragment shader to the shader program
	glLinkProgram(shaderProgram2); // links the attached shaders together
	...
	int indices[] = {
		...
		2, 3, 4,//second traingle
		4, 5, 6, // third triangle
		4, 7, 2 //fourth triangle
	};
	//-----------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------
										//VAO//
